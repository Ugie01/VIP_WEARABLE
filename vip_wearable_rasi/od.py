...
def run_object_detection(g_FRAME_OK, g_OD_PROCESSING, g_OBJECT_EXIST, g_ANGLE_OK, shared_queue):
    ...
    try:
        while True:
            if not g_FRAME_OK.value or not g_OD_PROCESSING.value or not g_ANGLE_OK.value:
                # 만약 프레임은 들어왔는데(g_OD_PROCESSING이 True) 경로 이탈(g_ANGLE_OK가 False) 상태라면,
                # 메인의 신호를 소거해 주고 루프를 스킵합니다.
                if g_OD_PROCESSING.value and not g_ANGLE_OK.value:
            ...
            # 각도 보정이 완료되지 않은 상태에서는 OD 연산을 수행하지 않고 대기합니다.
            time.sleep(0.002)
