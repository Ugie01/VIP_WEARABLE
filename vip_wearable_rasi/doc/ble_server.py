...
# 💡 [라즈베리파이 5 변경점 반영]: 하드웨어 표준 포트인 /dev/ttyAMA0로 수정
...
async def main():
    ...
    # 💡 [라즈베리파이 5 핵심 보완]: 기본 블루투스 어댑터(hci0) 객체를 직접 명시적으로 가져옴
    from bluez_peripheral.util import Adapter
    adapter = await Adapter.get_first(bus)
    if adapter is None:
        print("[오류] 시스템에서 블루투스 어댑터(hci0)를 감지할 수 없습니다. 하드웨어를 점검하십시오.")
        return
        
    service = TrackerGattService()
    # 💡 [버그 우회]: register 호출 시 추출한 수동 어댑터 인자를 전달하여 인터페이스 이탈 차단
    await service.register(bus, adapter=adapter) 
    
    # 💡 [라즈베리파이 5 명칭 매핑]: 광고 디바이스 명칭을 RaspberryPi5_Guide로 갱신
    agent = Advertisement(
        "RaspberryPi5_Guide",
        [SERVICE_UUID],
        appearance=0,
    ...
    # 💡 광고 등록부에도 수동 어댑터 인자 주입
    await agent.register(bus, adapter=adapter) 
    
    print("==================================================")
    print("  라즈베리파이5 고속 BLE 가이드 서버 (하드웨어 수동 바인딩)")
    print("==================================================")
